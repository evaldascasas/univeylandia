...
//        if(isset($_GET['email']) && !empty($_GET['email']) AND isset($_GET['hash']) && &&!empty($_GET['hash'])){
        ...
        //  echo 'rip';
          ...
          $hash = $_GET['hash'];
          var_dump($_GET['hash']);
          $search = mysqli_query($connection, "SELECT email, hash, actiu FROM USUARI WHERE email='$email' AND hash='$hash' AND actiu='0';");
          var_dump($search);
          $match = mysqli_num_rows($search);
          var_dump($match);
          if($match > 0){
            mysqli_query($connection, "UPDATE USUARI SET actiu='1' WHERE email='$email' AND hash='$hash' AND actiu='0'");
            echo 'Valida complet!';
          }else{
            echo 'Valida failed :(';
          }
